from sqlalchemy import Integer, String, Text, ForeignKey, DECIMAL, TIMESTAMP, DateTime, Boolean, func
...
class Employee(Base):
    ...
    def __repr__(self):
        return f"<Employee(id={self.id}, employee_number='{self.employee_number}', first_name='{self.first_name}', " \
               f"last_name='{self.last_name}', email='{self.email}', role_id={self.role_id})>"
...
class Customer(Base):
    ...
    def __repr__(self):
        return f"<Customer(id={self.id}, first_name='{self.first_name}', last_name='{self.last_name}', " \
               f"email='{self.email}', phone='{self.phone}', company='{self.company}', " \
               f"sale_contact_id={self.sale_contact_id})>"
...
class Contract(Base):
    ...
    def __repr__(self):
        return f"<Contract(id={self.id}, customer_id={self.customer_id}, sale_contact_id={self.sale_contact_id}, " \
               f"total_amount={self.total_amount}, remaining_amount={self.remaining_amount}, signed={self.signed})>"
...
class Event(Base):
    ...
    def __repr__(self):
        return f"<Event(id={self.id}, name='{self.name}', contract_id={self.contract_id}, " \
               f"support_contact_id={self.support_contact_id}, start_date='{self.start_date}', " \
