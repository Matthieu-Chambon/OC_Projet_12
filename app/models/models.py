...
class Contract(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    customer_details: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    def __repr__(self):
        return f"<Contract(id={self.id}, customer_id={self.customer_id}, employee_id={self.employee_id}, total_amount={self.total_amount}, remaining_amount={self.remaining_amount}, signed={self.signed})>"
...
class Event(Base):
    ...
    def __repr__(self):
        return f"<Event(id={self.id}, name='{self.name}', contract_id={self.contract_id}, start_date={self.start_date}, end_date={self.end_date}, location='{self.location}', attendees={self.attendees})>"
